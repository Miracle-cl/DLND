...
# 模型对数据的预测效果比较差，我认为是数据集的拆分又问题，数据按时间排序，从末端截取测试集和验证集，使得训练集上并没有考虑到11，12月的自行车租赁情况，数据在统计学上并不是随机的。应该先将数据按数据打乱，在进行数据的分割和模型的训练。
...
data_path = 'Bike-Sharing-Dataset/hour.csv'
rides = pd.read_csv(data_path)
rides = rides.sample(frac=1).reset_index(drop=True) # 对数据进行重新洗牌shuffle
rides.head(10)

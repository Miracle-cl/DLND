...
frequency_word = {word: freq/total_counts for word, freq in word_counts.items()}
prob_drop = {word: 1-np.sqrt(threshold/frequency_word[word]) for word in frequency_word}
# The final subsampled word list
# train_words = [word for word in int_words if prob_drop[word] < random.random()]
...
def get_target(words, idx, window_size=5):
    ...
    target = set(words[start: idx] + words[idx+1: end+1])
...
with train_graph.as_default():
    ...
    embedding = tf.random_uniform((n_vocab, n_embedding), minval=-1, maxval=1)

...
# for train_idx, val_test_idx in ss.split(codes, labels_vecs):
#     train_x, vt_x = codes[train_idx], codes[val_test_idx]
#     train_y, vt_y = labels_vecs[train_idx], labels_vecs[val_test_idx]

train_idx, val_test_idx = next(ss.split(codes, labels_vecs))
half_num = int( len(val_test_idx) / 2 )
val_idx, test_idx = val_test_idx[:half_num], val_test_idx[half_num:]

train_x, val_x, test_x = codes[train_idx], codes[val_idx], codes[test_idx]
train_y, val_y, test_y = labels_vecs[train_idx], labels_vecs[val_idx], labels_vecs[test_idx]
...
logits = tf.layers.dense(fc, 5, activation=None) # output layer logits
cost = tf.losses.softmax_cross_entropy(labels_, logits) # cross entropy loss
...
optimizer = tf.train.AdamOptimizer(learning_rate=0.001).minimize(cost) # training optimizer
